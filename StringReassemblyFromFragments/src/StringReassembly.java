...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        // Adds to str2 substring of str1 that does not overlap
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        // Iterates through each element of strSet
        for (String s : strSet) {
            // Verifies if str is a substring of any element in strSet
            if (s.indexOf(str) != -1) {
                // Returns making no changes
                return;
            }
        }

        // If str is not a substring of strSet, adds str to set
        strSet.add(str);
        // Initializes new temporary set
        Set<String> temp = new Set1L<>();
        /*
         * Iterates through each element of strSet and decides if element should
         * be added to temp. If the element is equal to str, then it is added to
         * temp. Otherwise, if the element is not a substring of str, it is also
         * added to temp.
         */
        for (String s : strSet) {
            if (s.equals(str)) {
        ...
        // Clears original strSet
        strSet.clear();
        // Adds all elements of temp to update strSet
        strSet.add(temp);
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // Create an empty set to store the unique lines from input
        Set<String> setStr = new Set2<>();

        // Read each line from input until the end of stream
        while (!input.atEOS()) {
            String str = input.nextLine();
            /*
             * Add the line to the set only if it is not a substring of any line
             * already in the set
             */
            addToSetAvoidingSubstrings(setStr, str);
        }
        // Returns set of unique lines
        return setStr;
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        // Iterates over each character in text string
        for (int i = 0; i < text.length(); i++) {
            // Checks for ~ character at each position in string
            if (text.charAt(i) == '~') {
                // If ~ found, goes to new line
                out.println();
            } else {
